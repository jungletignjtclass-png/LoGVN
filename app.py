...
# ---------------------------------------------------------------------------
# OSRM IMPORT - tương thích cả repo cũ có osrm_public_routing.py và repo hiện
# tại dùng routing.py. Không để thiếu 1 helper làm app chết ngay khi import.
# ---------------------------------------------------------------------------
try:
...
if "get_osrm_matrices_safe" not in globals():
    @st.cache_data(show_spinner=False, ttl=3600)
    def get_osrm_matrices_safe(coordinates, base_url=DEFAULT_OSRM_BASE_URL, batch_size=4):
        """Tạo ma trận OSRM đầy đủ bằng các block nhỏ.

        batch_size là số block mỗi chiều (VD 4 -> tối đa 16 request cho
        khoảng 100 điểm). Mỗi request chỉ chứa một block nguồn + một block
        đích, tránh URL quá dài/HTTP 414 của OSRM public.
        """
        coords = [tuple(c) for c in coordinates]
        n = len(coords)
        if n == 0:
            return SimpleNamespace(
                distance_matrix_m=[], duration_matrix_s=[], source="OSRM", warning=None
            )
        if n == 1:
            return SimpleNamespace(
                distance_matrix_m=[[0.0]], duration_matrix_s=[[0.0]],
                source="OSRM", warning=None
            )

        # batch_size = số block mỗi chiều, không phải số điểm trong block.
        num_blocks = max(1, int(batch_size))
        block_size = max(1, math.ceil(n / num_blocks))
        ...
        for src_idx in blocks:
            for dst_idx in blocks:
                # Ghép source + destination để mỗi Table request nhỏ.
                combined = [coords[i] for i in src_idx] + [coords[j] for j in dst_idx]
                local = get_osrm_matrices(
                    tuple(combined),
                    base_url=base_url,
                    allow_haversine_fallback=False,
                )
                split = len(src_idx)
                for si, gi in enumerate(src_idx):
                    for dj, gj in enumerate(dst_idx):
                        dist[gi][gj] = float(local.distance_matrix_m[si][split + dj])
                        dur[gi][gj] = float(local.duration_matrix_s[si][split + dj])
                if local.warning:
                    warnings.append(local.warning)

        return SimpleNamespace(
            distance_matrix_m=dist,
            duration_matrix_s=dur,
            source="HAVERSINE_FALLBACK" if used_fallback else "OSRM",
