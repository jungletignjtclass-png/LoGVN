...
from dynamic_routing import interpolate_along_route, haversine_m as _haversine_m_local
...
class RouteLeg:
    ...
    detoured: bool = False  # True nếu geometry này đã được thay bằng tuyến
    # tránh sự cố (xem incident_routing.reroute_leg_around_incident) - cạnh
    # logic (from_node, to_node) vẫn có thể nằm trong blocked_edges, nhưng
    # KHÔNG được coi là "bị chặn" nữa vì đường đi thực tế của leg này đã né
    # qua sự cố.
...
class FleetSimulator:
    ...
    # ------------------------------------------- sự cố: xe bị ảnh hưởng
    def is_route_affected(self, vehicle_id: int, a: int, b: int) -> int | None:
        """Trả về leg_idx ĐẦU TIÊN (tính từ leg hiện tại trở đi, bao gồm leg
        hiện tại) trùng với cạnh (a, b) trên tuyến còn lại của xe, hoặc None
        nếu tuyến còn lại KHÔNG đi qua cạnh này. Các leg đã đi qua (index nhỏ
        hơn leg_idx hiện tại, tức "phía sau xe") KHÔNG được tính -> đúng yêu
        cầu "sự cố nằm phía sau xe thì không cần re-route"."""
        truck = self.trucks.get(vehicle_id)
        if truck is None:
            return None
        target = frozenset((a, b))
        for idx in range(truck.leg_idx, len(truck.legs)):
            leg = truck.legs[idx]
            if frozenset((leg.from_node, leg.to_node)) == target:
                return idx
        return None

    def find_affected_trucks(self, a: int, b: int) -> dict:
        """vehicle_id -> leg_idx bị ảnh hưởng, chỉ xét các xe đang hoạt động
        (bỏ qua xe hỏng/đã hoàn thành, vì các xe đó không còn di chuyển)."""
        affected: dict[int, int] = {}
        for vid, truck in self.trucks.items():
            if truck.status in (STATE_BROKEN, STATE_COMPLETED):
                continue
            idx = self.is_route_affected(vid, a, b)
            if idx is not None:
                affected[vid] = idx
        return affected

    def remaining_route_length_m(self, vehicle_id: int) -> float:
        """Tổng chiều dài (m) các leg CÒN LẠI (từ leg hiện tại trở đi) của 1
        xe, dùng để so sánh chi phí trước/sau khi tái định tuyến vì sự cố."""
        truck = self.trucks.get(vehicle_id)
        if truck is None:
            return 0.0
        total = 0.0
        for idx in range(truck.leg_idx, len(truck.legs)):
            geometry = truck.legs[idx].geometry
            for k in range(len(geometry) - 1):
                lat1, lon1 = geometry[k]
                lat2, lon2 = geometry[k + 1]
                total += _haversine_m_local(lat1, lon1, lat2, lon2)
        return total

    def set_leg_geometry(self, vehicle_id: int, leg_idx: int, new_geometry: list) -> bool:
        """Thay geometry của 1 leg cụ thể (VD sau khi tìm được tuyến né sự
        cố) và đánh dấu leg đó là `detoured=True` để tick loop không còn coi
        leg này là bị chặn. Nếu leg_idx CHÍNH LÀ leg xe đang đi dở
        (leg_idx == truck.leg_idx), reset leg_progress_m về 0 vì geometry mới
        không còn tương ứng 1-1 với quãng đường xe đã đi trên geometry cũ -
        xe được coi là "tiếp tục từ vị trí hiện tại" theo geometry mới."""
        truck = self.trucks.get(vehicle_id)
        if truck is None or not (0 <= leg_idx < len(truck.legs)):
            return False
        old_leg = truck.legs[leg_idx]
        truck.legs[leg_idx] = RouteLeg(
            from_node=old_leg.from_node, to_node=old_leg.to_node,
            geometry=list(new_geometry), detoured=True,
        )
        if leg_idx == truck.leg_idx:
            truck.leg_progress_m = 0.0
    ...
    def _tick_truck(self, truck: Truck, accel_seconds: float) -> None:
        ...
        # Nếu leg này ĐÃ được reroute quanh sự cố (detoured=True), xe được
        # phép tiếp tục dù cặp node (from_node, to_node) vẫn nằm trong
        # blocked_edges - vì đường đi thực tế (geometry) không còn dùng đoạn
        # đường bị chặn nữa.
        if self._edge_blocked(leg.from_node, leg.to_node) and not leg.detoured:
            if truck.status != STATE_BLOCKED:
